<script lang="ts">
    ...
    // Holds the full recipe objects stored in favorites
        ...
        if (saved) {
            try {
                ...
                if (Array.isArray(parsed)) {
                    // Ensure we only store valid recipe objects with an 'id'
    ...
    function removeFavorite(recipeId: number) {
        // Remove recipe from reactive state
        favoriteRecipes = favoriteRecipes.filter((recipe) => recipe.id !== recipeId);

        // Sync updated array back to localStorage
...
<bw-container pd="20px">
    ...
    {:else}
        <bw-container primary="true" pd="3rem" radius="15px" border="2px solid white" mgV="2rem" class="h-screen">
